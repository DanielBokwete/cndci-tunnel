  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-4">
        <button onClick={() => setFiltre('tous')} className={`text-xs rounded-full px-3 py-1.5 border transition ${filtre === 'tous' ? 'bg-white text-black border-white' : 'border-gray-800 text-gray-400 hover:border-gray-600'}`}>
          Tous
        </button>
        <button onClick={() => setFiltre('confirmes')} className={`text-xs rounded-full px-3 py-1.5 border transition ${filtre === 'confirmes' ? 'bg-green-600 text-white border-green-600' : 'border-gray-800 text-gray-400 hover:border-gray-600'}`}>
          Confirmés
        </button>
        <button onClick={() => setFiltre('non_confirmes')} className={`text-xs rounded-full px-3 py-1.5 border transition ${filtre === 'non_confirmes' ? 'bg-gray-700 text-white border-gray-700' : 'border-gray-800 text-gray-400 hover:border-gray-600'}`}>
          Non confirmés
        </button>
      </div>
      ...
      <div className="space-y-2">
        {prospectsFiltres.map((p) => {
          ...
          return (
            <div key={p.id} className={`border rounded-xl px-4 py-3 flex items-center justify-between gap-4 ${p.confirme ? 'border-green-800 bg-green-950/20' : 'border-gray-800'}`}>
              <div>
                ...
                <p className="text-sm text-gray-500">{p.whatsapp}{p.email ? ` · ${p.email}` : ''}</p>
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <button onClick={() => copier(p.whatsapp, p.id)} className="text-xs bg-gray-900 border border-gray-800 rounded-lg px-3 py-1.5 hover:border-gray-600 transition">
